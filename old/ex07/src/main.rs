    ...
    for (race_num, chunk) in SITES.chunks(3).enumerate() {
        ...
        tokio::select! {
            len0 = page_len(site0.unwrap()), if site0.is_some() => {
                println!("site0 won race {}, {} {} bytes", race_num, site0.unwrap(), len0?);
            }
            len1 = page_len(site1.unwrap()), if site1.is_some() => {
                println!("site1 won race {}, {} {} bytes", race_num, site1.unwrap(), len1?);
            }
            len2 = page_len(site2.unwrap()), if site2.is_some() => {
                println!("site2 won race {}, {} {} bytes", race_num, site2.unwrap(), len2?);
            }
        }
    }
